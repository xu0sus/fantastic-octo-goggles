...
async def setup_hook():
    """Official discord.py-style startup hook.

    This is intentionally assigned directly to commands.Bot.setup_hook rather
    than registered with @bot.event. The CommandTree is ready here, and the
    translator is installed before any command synchronization occurs.
    """
    ...
    # Keep the global tree valid as well; guild sync below is what makes
    # commands appear immediately during development/testing.
    try:
        synced = await bot.tree.sync()
        print(f"🌐 Global command sync: {len(synced)} command(s)")
    except Exception as exc:
        print(f"❌ Global command sync failed: {type(exc).__name__}: {exc}")

# commands.Bot already provides the CommandTree; assigning setup_hook directly
# is the supported pattern for a Bot instance that is not subclassed.
bot.setup_hook = setup_hook

@bot.event
async def on_ready():
    if not getattr(bot, "_guild_commands_synced", False):
        local_commands = bot.tree.get_commands()
        print(f"📦 Local application commands: {len(local_commands)}")
        for guild in bot.guilds:
            try:
                bot.tree.copy_global_to(guild=guild)
                synced = await bot.tree.sync(guild=guild)
                print(f"✅ Guild command sync: {len(synced)} -> {guild.name} ({guild.id})")
            except Exception as exc:
                print(f"❌ Guild command sync failed: {guild.name} ({guild.id}) | {type(exc).__name__}: {exc}")
